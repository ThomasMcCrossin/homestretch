    ...
    if schedule_8 and isinstance(schedule_8.get("classes"), dict) and schedule_8["classes"]:
        ...
        if assets:
            ...
            for asset in assets:
                asset_rows.append(
                    [
                        ...
                        money(int(asset.get("total_cost_dollars") or 0)),
                        str(asset.get("aii_eligible") or ""),
                        str(asset.get("aii_factor") or ""),
                    ]
                )
            parts.append("### Schedule 8 asset additions (audit trail)")
            parts.append(md_table(["Asset ID", "Description", "Date", "Class", "Cost", "AIIP?", "AII factor"], asset_rows))
            parts.append("")
            ...
            for asset in assets:
                ...
                aii_eligible = str(asset.get("aii_eligible") or "").strip().lower() in ("yes", "true", "1")
                aii_factor = str(asset.get("aii_factor") or "").strip()
                base_factor = str(asset.get("first_year_base_factor") or "").strip()
                # For UFile: additions must land in exactly one bucket. Default to AIIP if eligible, else non-accelerated.
                aiip_add = cost if aii_eligible else 0
                non_acc_add = 0 if aii_eligible else cost
                ufile_rows.append(
                    [
                        str(asset.get("cca_class") or ""),
                        str(asset.get("description") or ""),
