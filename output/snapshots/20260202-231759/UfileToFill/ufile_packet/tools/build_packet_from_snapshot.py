...
def load_internet_sales_percent_by_fy(snapshot_dir: Path) -> dict[str, float]:
    """
    Load per-FY internet revenue % derived from Shopify "Net payments by gateway" (sales channels).

    Source file is generated by scripts/83_audit_shopify_gateway_vs_payouts.py and should be present in snapshots.
    """

    path = snapshot_dir / "shopify_internet_sales_percent_by_fy.csv"
    if not path.exists():
        return {}
    out: dict[str, float] = {}
    with path.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not fy:
                continue
            raw = str(row.get("percent_gross_revenue_from_internet") or "").strip()
            if not raw:
                continue
            try:
                out[fy] = float(Decimal(raw))
            except Exception:
                continue
    return out
            ...
            out.append(
                {
                    ...
                    "aii_eligible": str(row.get("aii_eligible") or "").strip(),
                    "aii_factor": str(row.get("aii_factor") or "").strip(),
                    "first_year_base_factor": str(row.get("first_year_base_factor") or "").strip(),
                    "half_year_rule": str(row.get("half_year_rule") or "").strip(),
                    "half_year_applied": str(row.get("half_year_applied") or "").strip(),
...
def update_year_section(
    ...
    *,
    snapshot_dir: Path,
    fy: str,
    internet_percent_by_fy: dict[str, float] | None = None,
) -> None:
    ...
    # Identification screen: keep % gross revenue from internet aligned to Shopify net payments by gateway.
    percent = (internet_percent_by_fy or {}).get(fy)
    if percent is not None:
        ident = year_screens.get("identification_of_corporation", {})
        if not isinstance(ident, dict):
            ident = {}
        internet = ident.get("internet_business", {})
        if not isinstance(internet, dict):
            internet = {}
        internet["percent_gross_revenue_from_internet"] = float(percent)
        ident["internet_business"] = internet
        year_screens["identification_of_corporation"] = ident
...
def main() -> int:
    ...
    internet_percent_by_fy = load_internet_sales_percent_by_fy(snapshot_dir)
    for fy in fys:
        update_year_section(packet, snapshot_dir=snapshot_dir, fy=fy, internet_percent_by_fy=internet_percent_by_fy)
