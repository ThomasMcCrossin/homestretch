...
def half_year_applies(asset: Asset, afu_date: date) -> bool:
    ...
    - If half_year_rule is true and aii_eligible is true with AFU in 2024-01-01..2027-12-31,
      the AII suspension removes the half-year rule.
    - Otherwise, apply the half-year rule.
    """

    aii_window_start = date(2024, 1, 1)
    aii_window_end = date(2027, 12, 31)
    in_aii_window = aii_window_start <= afu_date <= aii_window_end
    if not asset.half_year_rule:
        return False
    if asset.aii_eligible and in_aii_window:
        return False
    return True
